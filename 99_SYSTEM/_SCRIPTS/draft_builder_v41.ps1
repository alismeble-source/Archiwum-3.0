# draft_builder_v41.ps1
# V4.1: Draft for RISK cases + attach last evidence links from LINKS.txt
...
function Get-LastEvidence([string]$linksFile, [int]$maxItems = 5){
    ...
    # LINKS.txt is in blocks separated by "----" (V3 format)
    ...
    foreach($ln in $lines){
        if ($ln.Trim() -eq "----") {
            if ($buf.Count -gt 0) { $blocks += ,($buf.ToArray()); $buf.Clear() }
        } else {
            $buf.Add($ln)
        }
    }
    if ($buf.Count -gt 0) { $blocks += ,($buf.ToArray()) }

    if ($blocks.Count -eq 0) { return @("BRAK: nie udało się odczytać bloków w LINKS.txt.") }
...
Get-ChildItem $CASES -Directory | ForEach-Object {
    ...
    @(
      ...
      "- TODO: czego oczekuję jako finał"
