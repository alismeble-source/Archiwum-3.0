# inbox_router_v3.ps1
# V3: normalized LINKS, audit-friendly records, API-ready fields
...
foreach($f in $files){
    ...
    $statusPath = Join-Path $casePath "STATUS.txt"

    if (Is-Risk $f.Name){
        "RISK" | Out-File -Encoding UTF8 $statusPath
    }
    $status = (Get-Content $statusPath | Select-Object -First 1)
    ...
    $links = Join-Path $casePath "LINKS.txt"
    @(
      ...
      "SHARE:  "
      "----"
    ) | Out-File -Append -Encoding UTF8 $links

    Log "FILE: $($f.Name) -> TYPE=$type STATUS=$status -> $dest"
